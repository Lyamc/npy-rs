...
impl<Row: Serializable> OutFile<Row> {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> io::Result<Self> {
        let dtype = Row::dtype();
        if let DType::Plain { shape, .. } = &dtype {
            assert!(shape.is_empty(), "plain non-scalar dtypes not supported");
        }
        ...
        padding.extend(
            &::std::iter::repeat_n(b' ', 15 - ((header.len() + 10) % 16)).collect::<Vec<_>>(),
        );
        padding.extend(b"\n");
    ...
    fn close_(&mut self) -> io::Result<()> {
        ...
        self.fw.write_all(&b",), }"[..])?;
        self.fw.write_all(
            &::std::iter::repeat_n(b' ', FILLER.len() - length.len()).collect::<Vec<_>>(),
        )?;
        Ok(())
    }
